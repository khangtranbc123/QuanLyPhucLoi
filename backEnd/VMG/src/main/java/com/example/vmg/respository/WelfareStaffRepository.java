...
public interface WelfareStaffRepository extends JpaRepository<WelfareStaff, Long> {
    ...
    @Query("select x from WelfareStaff x where x.status = 2 AND x.staff.id =:id ")
    List<WelfareStaff> findByStaffRegisterId(Long id);
    ...
    @Query("select distinct x.staff from WelfareStaff x where x.status = 2 ")
    public List<WelfareStaff> getStaffRegister();
